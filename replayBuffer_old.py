...
def make_sample_her_transitions(replay_strategy, replay_k, reward_fun):
    ...
    def _sample_her_transitions(episode_batch, batch_size_in_transitions, current_size):
        """episode_batch is {key: array(buffer_size x T x dim_key)}
        """
        T = episode_batch['actions'].shape[1]
        rollout_batch_size = current_size
        ...
        #for key, value in episode_batch.items():
         #   print(f"{key}: shape {value.shape}, sample data {value[:2]}")    
        ...
        future_ag = episode_batch['next_observations'][episode_idxs[her_indexes], future_t]
        transitions['goals'][her_indexes] = future_ag[:, [4,5]]
        ...
        reward_params = {k: transitions[k] for k in ['next_observations', 'goals']}
        reward_params['info'] = info
        #print(reward_params['next_observations'][:, [4,5]].shape, transitions['actions'].shape, reward_params['goals'].shape)
        transitions['rewards'] = reward_fun(achieved_goal=reward_params['next_observations'][:, [4,5]],
        ...
        #print("\n", transitions['observations'].shape)
...
class PrioritizedHERReplayBuffer:
    ...
    def add(self, obs, action, reward, next_obs, goal, done):
        """
    ...
    def sample(self, batch_size, beta=0.4):
        ...
        with self.lock:
            ...
            #print(self.obs)
            valid_rollouts = self.size // self.T
            episode_batch = {
                'observations': self.obs[:self.size].reshape(valid_rollouts, self.T, self.obs_dim),  
                'actions': self.actions[:self.size].reshape(valid_rollouts, self.T, self.action_dim),
                'rewards': self.rewards[:self.size].reshape(valid_rollouts, self.T),
                'next_observations': self.next_obs[:self.size].reshape(valid_rollouts, self.T, self.obs_dim),
                'goals': self.goals[:self.size].reshape(valid_rollouts, self.T, self.goal_dim),
                'dones': self.dones[:self.size].reshape(valid_rollouts, self.T),
            }

            
            transitions, _ = self.her_sampler(episode_batch, batch_size, valid_rollouts)
